...
interface FilterBarProps {
  ...
  queryParams: ReviewsParams;
  updateFilters: (filter: string, value: string) => void;
  resetFilters: () => void;
}

export const ReviewFilterBar: React.FC<FilterBarProps> = ({ searchTerm, setSearchTerm, queryParams, updateFilters, resetFilters }) => {
  return (
    <div className="flex flex-wrap items-center gap-3 bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
      <div className="relative">
        ...
        <Input
          ...
          className="w-60 pl-9 rounded-lg"
        />
      </div>
      <Select value={queryParams.sortOrder || ""} onValueChange={(value) => updateFilters("sortOrder", value)}>
