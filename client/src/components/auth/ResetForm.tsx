...
export function ResetPasswordForm({ className, ...props }: React.ComponentProps<"form">) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      setLoading(true);
      const response = await resetPassword({ email, newPassword, confirmNewPassword, otp });
    ...
    <>
      <form className={cn("flex flex-col gap-4", className)} {...props} onSubmit={handleSubmit}>
        <div className="flex flex-col items-center gap-2 text-center">
          <p className=" text-sm text-balance text-white">Enter the details below to reset your password</p>
        </div>
        <div className="grid gap-6">
          ...
          <div className="grid gap-3">
            ...
            <InputOTP id="otp" maxLength={6} pattern={REGEXP_ONLY_DIGITS} value={otp} onChange={(value) => setOtp(value)}>
              <InputOTPGroup>
                {[...Array(6)].map((_, i) => (
          ...
          <div className="grid gap-3">
            ...
            <div className="relative">
              ...
              />
              <div onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="cursor-pointer absolute right-3 top-1/2 -translate-y-1/2">
                {showPassword ? <EyeOff /> : <Eye />}
              </div>
            </div>
          </div>
